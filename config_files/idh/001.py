...
class configs(DefaultConfigs):
    ...
    def __init__(self, dim): 
        ...
        DefaultConfigs.__init__(self, self.model, self.dim, molecular_status = 'IDH')
        ...
        self.channels = range(len(self.config["training_modalities"]))
        self.n_channels = len(self.config["training_modalities"])

        # patch_size to be used for training. pre_crop_size is the patch_size before data augmentation.
        self.pre_crop_size_2D = [128, 128]
        self.patch_size_2D = [128, 128]
        ...
        'do_rotation':True, 'angle_x': (-0.25, 0.25), 'angle_y': (0., 0), 'angle_z': (0., 0),
        'border_mode_data': 'nearest'}

        if self.dim == 3:
            self.da_kwargs['angle_x'] = (0, 0.0)
            self.da_kwargs['angle_y'] = (0, 0.0) #must be 0!!
            self.da_kwargs['angle_z'] = (-0.25, 0.25)
        ...
        self.mrcnn_class_loss_weights = compute_class_weights(self.df_filtered, self.config)
        self.train_augmentation = ['mirror', 'spatial', 'contrast'] # If True, data augmentation is performed during training
    ...
    def add_mrcnn_configs(self):
        ...
        self.window = np.array([0, 0, self.patch_size[0], self.patch_size[1], 0, self.patch_size_3D[2]]) # for 3d
        self.scale = np.array([self.patch_size[0], self.patch_size[1], self.patch_size[0], self.patch_size[1],
                               self.patch_size_3D[2], self.patch_size_3D[2]]) # for 3d
