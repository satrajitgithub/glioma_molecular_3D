...
class Evaluator():
    ...
    def evaluate_predictions(self, results_list, monitor_metrics=None):
        ...
        # # at this point, results_list = output of test_predictor.predict_test_set()
        # self.logger.debug("results_list inside evaluate_predictions() ")
        ...
        for match_iou in self.cf.ap_match_ious:
            ...
            for cl in list(self.cf.class_dict.keys()):
                ...
                for pix, pid in enumerate(pid_list):
                    ...
                    # batch_elements_list[pix] has, for a particular patient 'pix', pred bboxes/pred_class (box_type = 'det') + gt bbox/gt_class (box_type = 'gt')

                    for bix, b_boxes_list in enumerate(batch_elements_list[pix]):

                        # self.logger.debug(f"b_boxes_list for class = {cl}, pid = {pid}")
                        # pprint.pprint(b_boxes_list)

                        b_tar_boxes = np.array([box['box_coords'] for box in b_boxes_list if (box['box_type'] == 'gt' and box['box_label'] == cl)]) # gt box coords
                        b_cand_boxes = np.array([box['box_coords'] for box in b_boxes_list if (box['box_type'] == 'det' and box['box_pred_class_id'] == cl)]) # candidate box coords
                        b_cand_scores = np.array([box['box_score'] for box in b_boxes_list if (box['box_type'] == 'det' and box['box_pred_class_id'] == cl)]) # candidate box score


                        # self.logger.debug("b_tar_boxes")
                        # pprint.pprint(b_tar_boxes)
                        # self.logger.debug("b_cand_boxes")
                        # pprint.pprint(b_cand_boxes)
                        # self.logger.debug("b_cand_scores")
                        # pprint.pprint(b_cand_scores)
        ...
        # self.logger.debug("self.test_df")
        # pprint.pprint(self.test_df)
