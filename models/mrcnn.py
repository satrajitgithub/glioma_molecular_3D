...
def compute_mrcnn_class_loss(target_class_ids, pred_class_logits, class_weights = None):
    """
    ...
    :return: loss: torch 1D tensor.
    """
    # print("target_class_ids", target_class_ids.long())
    # print("pred_class_logits", pred_class_logits)

    class_weights_tensor = torch.FloatTensor(class_weights).cuda() if class_weights else None
    # print(f"pred = {pred_class_logits}, gt = {target_class_ids.long()}")

    if 0 not in target_class_ids.size():
        loss_unreduced = F.cross_entropy(pred_class_logits, target_class_ids.long(), weight = class_weights_tensor, reduction = 'none')
        loss = torch.mean(loss_unreduced)
        # loss_no_weight = F.cross_entropy(pred_class_logits, target_class_ids.long(), weight = None)
        # print(f"loss went from {loss_no_weight} --> {loss}")
    else:
        loss = torch.FloatTensor([0.]).cuda()
...
class net(nn.Module):
    ...
    def train_forward(self, batch, is_validation=False):
        ...
        # print(f"self.cf.dim = {self.cf.dim}, is_validation = {is_validation}, gt_boxes = {gt_boxes}")
        ...
        # print("self.cf.mrcnn_class_loss_weights", self.cf.mrcnn_class_loss_weights)
        mrcnn_class_loss = compute_mrcnn_class_loss(target_class_ids, mrcnn_class_logits, class_weights = self.cf.mrcnn_class_loss_weights)
        ...
        results_dict['monitor_values'] = {'loss': loss.item(), 
                                          'class_loss': mrcnn_class_loss.item(),
                                          'mrcnn_bbox_loss': mrcnn_bbox_loss.item(),
