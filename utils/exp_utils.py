...
def prep_exp(dataset_path, exp_path, cfg_name, dim, use_stored_settings=True, is_training=True):
    """
    ...
    if is_training:
        ...
        if not os.path.exists(exp_path):
            ...
            os.mkdir(os.path.join(exp_path, 'code_cfg_snapshot'))
            shutil.copy(os.path.join(dataset_path, cfg_name), os.path.join(exp_path, 'code_cfg_snapshot', cfg_name))
            shutil.copy('default_configs.py', os.path.join(exp_path, 'code_cfg_snapshot', 'default_configs.py'))


        if use_stored_settings:
            shutil.copy('default_configs.py', os.path.join(exp_path, 'code_cfg_snapshot', 'default_configs.py'))
            cf_file = import_module('cf', os.path.join(exp_path, 'code_cfg_snapshot', cfg_name))
            cf = cf_file.configs(dim)
            # only the first process copies the model selcted in configs to exp_path.
            if not os.path.isfile(os.path.join(exp_path, 'code_cfg_snapshot', 'model.py')):
                shutil.copy(cf.model_path, os.path.join(exp_path, 'code_cfg_snapshot', 'model.py'))
                shutil.copy(os.path.join(cf.backbone_path), os.path.join(exp_path, 'code_cfg_snapshot', 'backbone.py'))
            ...
            shutil.copy(os.path.join(exp_path, 'code_cfg_snapshot', 'model.py'), tmp_model_path)
            shutil.copy(os.path.join(exp_path, 'code_cfg_snapshot', 'backbone.py'), tmp_backbone_path)
            ...
            cf_file = import_module('cf', os.path.join(dataset_path, cfg_name))
            cf = cf_file.configs(dim)
            shutil.copy(cf.model_path, os.path.join(exp_path, 'code_cfg_snapshot', 'model.py'))
            shutil.copy(cf.backbone_path, os.path.join(exp_path, 'code_cfg_snapshot', 'backbone.py'))
            shutil.copy('default_configs.py', os.path.join(exp_path, 'code_cfg_snapshot', 'default_configs.py'))
            shutil.copy(os.path.join(dataset_path, cfg_name), os.path.join(exp_path, 'code_cfg_snapshot', cfg_name))
...
class ModelSelector:
    ...
    def run_model_selection(self, net, optimizer, monitor_metrics, epoch):
        ...
        # check if monitor_metrics['val']['monitor_values'] are not like = [[], [], [], []..] which might be the case if validation is cfg.do_validation = False
        # If that is the case, then instead use training metrics for choosing best models
        if any(monitor_metrics['val']['monitor_values']):
            ...
            warnings.warn("monitor_metrics['val']['monitor_values'] are all empty (probably because cfg.do_validation = False)."
                            "Using monitor_metrics['train']['monitor_values'] instead for saving best model checkpoints.")
            
            non_nan_scores = np.mean(np.array([[0 if ii is None else ii for ii in monitor_metrics['train'][sc]] for sc in self.cf.model_selection_criteria]), 0)
        ...
        if epoch in epoch_ranking[:self.cf.save_n_models]:
            ...
            self.logger.info("saving current epoch {} at rank {}".format(epoch, np.argwhere(epoch_ranking == epoch)))
...
def create_csv_output(results_list, cohort_suffix, model_suffix, cf, logger):
    """
    ...
    """
    path_to_csv = os.path.join(cf.exp_dir, f'results_{model_suffix}+{cohort_suffix}.csv')
    logger.info('creating csv output file at {}'.format(path_to_csv))
    ...
    try:
        ...
        fold = 'hold_out'
    predictions_df.to_csv(path_to_csv, index=False)
