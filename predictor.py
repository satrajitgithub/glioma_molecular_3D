...
class Predictor:
    ...
    def __init__(self, cf, net, logger, mode):
        ...
        if self.mode == 'test':
            try:
                ...
                warnings.warn('No epoch ranking file in fold directory. Will try to load model weights from: last_checkpoint/params.pth')
    ...
    def predict_patient(self, batch):
        ...
        """
        self.logger.info('evaluating patient: {} for fold {} '.format(batch['session_name'], self.cf.fold))
    ...
    def predict_test_set(self, batch_gen, cohort_suffix, return_results=True, model_suffix = 'last'):
        """
        ...
        elif model_suffix == 'last':
            # use last 
            weight_paths = [os.path.join(self.cf.fold_dir, 'last_checkpoint', 'params.pth')]
        else:
            raise
        ...
        for rank_ix, weight_path in enumerate(weight_paths):
            ...
            with torch.no_grad():
                ...
                for _ in range(batch_gen['n_test']):
                    ...
                    self.logger.debug(f"{count}/{batch_gen['n_test']}")

                    batch = next(batch_gen['test'])
                    # self.logger.debug("batch")
                    # pprint.pprint(batch)

                    # # if want to run for specific pid during debug
                    # if batch['pid'] == '68':
                    ...
                    if rank_ix == 0:
                        ...
                        dict_of_patient_results[batch['pid']]['patient_bb_target'] = batch['patient_bb_target'] # gt bbox coords
                        dict_of_patient_results[batch['pid']]['patient_roi_labels'] = batch['patient_roi_labels'] # gt class_id (+1 to account for BG)

                    # call prediction pipeline and store results in dict.

                    # at this point, results_dict contains (for one patient) :
                    # 1. boxes - predicted boxes (with coords, score, pred_class_id)
                    # 2. seg_preds - predicted segmentation mask                    
                    results_dict = self.predict_patient(batch)
                    # self.logger.debug("results_dict['boxes']")
                    # pprint.pprint(results_dict['boxes'])
                    # print(results_dict['seg_preds'].shape)


                    dict_of_patient_results[batch['pid']]['results_list'].append(results_dict['boxes']) # all predicted bboxes for a patient are appended to dict_of_patient_results

                    # break

                    end_time = time.time()

                    print("[INFO] Time taken: {} seconds".format((end_time-start_time)))

        # at this point, dict_of_patient_results contains (for each patient) key=pid, value = p_dict, for each patient, where each p_dict contains:
        # 1. patient_bb_target - gt bbox coords
        # 2. patient_roi_labels - gt class_id (+1 to account for BG)
        # 3. results_list - all predicted bboxes(+pred class label) 
        # self.logger.debug("dict_of_patient_results")
        # pprint.pprint(dict_of_patient_results) 
        ...
        # # at this point, list_of_results_per_patient contains, for each patient, list of pred bboxes/pred_class (box_type = 'det') + gt bbox/gt_class (box_type = 'gt')
        # self.logger.debug("list_of_results_per_patient --> before pickling in predict_test_set()")
        # pprint.pprint(list_of_results_per_patient) 

        # save out raw predictions.
        out_string = f'raw_pred_boxes_hold_out_list_{model_suffix}+{cohort_suffix}' if self.cf.hold_out_test_set else f'raw_pred_boxes_list_{model_suffix}+{cohort_suffix}'
        ...
        if return_results:
            ...
            # self.logger.debug("list_of_results_per_patient before applying wcs")
            # pprint.pprint(list_of_results_per_patient) 

            # consolidate predictions.
            self.logger.info('applying wcs to test set predictions with iou = {} and n_ens = {}.'.format(self.cf.wcs_iou, self.n_ens))
            ...
            # # at this point, list_of_results_per_patient contains same as above, but number of predicted bboxes are much lesser by applying wcs
            # self.logger.debug("list_of_results_per_patient after applying wcs")
            # pprint.pprint(list_of_results_per_patient) 

            return list_of_results_per_patient


    def predict_single_test_patient(self, batch, return_results=True, use_last_model = None):
        """
        wrapper around test method, which loads multiple (or one) epoch parameters (temporal ensembling), loops through
        the test set and collects predictions per patient. Also flattens the results per patient and epoch
        and adds optional ground truth boxes for evaluation. Saves out the raw result list for later analysis and
        ...
        :return: (optionally) list_of_results_per_patient: list over patient results. each entry is a dict with keys:
                 ...
                 - 'seg_preds': not implemented yet. todo for evaluation of instance/semantic segmentation.
        """
        epoch_ranking = np.load(os.path.join(self.cf.fold_dir, 'epoch_ranking.npy'))[:1]

        if not use_last_model:
            # get paths of best weights
            weight_path = [os.path.join(self.cf.fold_dir, '{}_best_checkpoint'.format(epoch), 'params.pth') for epoch in epoch_ranking][0]
        else:
            # use last 
            weight_path = os.path.join(self.cf.fold_dir, 'last_checkpoint', 'params.pth')


        self.logger.info(('Loading weights from epoch:{}'.format(weight_path)))
        ...
        # consolidate predictions.
        self.logger.info('applying wcs to test set predictions with iou = {} and n_ens = {}.'.format(self.cf.wcs_iou, self.n_ens))
        wcs_input = [results_dict['boxes'], batch['pid'], self.cf.class_dict, self.cf.wcs_iou, self.n_ens]
        results_dict['boxes'] = apply_wbc_to_patient(wcs_input)[0]
    ...
    def load_saved_predictions(self, cohort_suffix, model_suffix, apply_wbc=False):
        """
        ...
        # corresponding to all CV-folds and flatten them.
            ...
            for fold in self.cf.folds:
                ...
                with open(os.path.join(fold_dir, f'raw_pred_boxes_hold_out_list_{model_suffix}+{cohort_suffix}.pickle'), 'rb') as handle:
        ...
        # # at this point, list_of_results_per_patient contains only predicted bboxes and pred_class BUT no gt because only 'det' type boxes are included above (*)
        # self.logger.debug("list_of_results_per_patient --> beginning of load_saved_predictions()")
        # pprint.pprint(list_of_results_per_patient)
        ...
        # # at this point, list_of_results_per_patient contains only predicted bboxes and pred_class BUT no gt
        # self.logger.debug("list_of_results_per_patient --> end of load_saved_predictions()")
        # pprint.pprint(list_of_results_per_patient)

        return list_of_results_per_patient


    def load_saved_predictions_and_gt(self, cohort_suffix, apply_wbc=False):
        """
        Modified from: <load_saved_predictions()>
        ...
        for fold in self.cf.folds:
            fold_dir = os.path.join(self.cf.exp_dir, 'fold_{}'.format(fold))
            with open(os.path.join(fold_dir, f'raw_pred_boxes_hold_out_list_{cohort_suffix}.pickle'), 'rb') as handle:
                fold_list = pickle.load(handle)
                pids = [ii[1] for ii in fold_list]
                boxes_list.append([ii[0] for ii in fold_list])
        list_of_results_per_patient = [[[[box for fold_list in boxes_list for box in fold_list[pix][0]]], pid] for pix, pid in enumerate(pids)]
        da_factor = 4 if self.cf.test_aug else 1
        n_ens = self.cf.test_n_epochs * da_factor * len(self.cf.folds)

        # at this point, list_of_results_per_patient contains predicted bboxes and pred_class + gt bbox and class

        # consolidate predictions.
        if apply_wbc:
            self.logger.info('applying wcs to test set predictions with iou = {} and n_ens = {}.'.format(self.cf.wcs_iou, n_ens))
            pool = Pool(processes=6)
            mp_inputs = [[ii[0], ii[1], self.cf.class_dict, self.cf.wcs_iou, n_ens] for ii in list_of_results_per_patient]
            list_of_results_per_patient = pool.map(apply_wbc_to_patient, mp_inputs, chunksize=1)
            pool.close()
        ...
        with open(os.path.join(fold_dir, f'results_list_{cohort_suffix}.pickle'), 'wb') as handle: pickle.dump(list_of_results_per_patient, handle)
    ...
    def data_aug_forward(self, batch):
        ...
        # self.logger.debug("[data_aug_forward] results_list")
        # pprint.pprint(results_list)

        # assert np.count_nonzero(results_list[0]['seg_preds']) > 0, "[data_aug_forward - before test_aug] Returned segmentation is all zero"

        if self.mode == 'test' and self.cf.test_aug:

            self.logger.debug("** TEST_AUG IS ON")
        ...
        # assert np.count_nonzero(results_dict['seg_preds']) > 0, "[data_aug_forward] Returned segmentation is all zero"
    ...
    def spatial_tiling_forward(self, batch, patch_crops=None, n_aug='0'):
        ...
        if patch_crops is not None:
            ...
            raise ValueError("[spatial_tiling_forward] Using patch_crops. Why is the data patched?")
    ...
    def batch_tiling_forward(self, batch):
        ...
        :return. results_dict: stores the results for one patient. dictionary with keys:
                 ...
                 - monitor_values (only in validation mode)
        """
        self.logger.info('forwarding patient with shape: {}'.format(batch['data'].shape))
